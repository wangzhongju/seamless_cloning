...
namespace customCV {
	...
	void Cloning::initialization(Mat &I, Mat &mask, Mat &wmask)
	{
		...
		//腐蚀
		erode(wmask, wmask, Kernel, Point(-1, -1), 3); // 3代表迭代三次

		// convertTo(OutputArray m, int rtype, double alpha=1, double beta=0) 负责Mat数据类型的转换，以及像素值m(x,y)=saturate_cast<rType>(α(∗this)(x,y)+β)
		// cvtColor()负责Mat通道即色彩空间的转换 CV_8U(0-255) CV_16U(0-65535) CV_32F(0-1)
		// imwrite执行时自动将图片深度转换为CV_8U，这就是为什么imshow与imwrite图片显示不一致的原因
	...
	void Cloning::array_product(Mat mat1, Mat mat2, Mat mat3)
	{
		...
		multiply(channels_temp2[2], mat3, channels_temp1[2]);  //计算两个数组按元素缩放的乘积，channels_temp1[2](I)=𝚜𝚊𝚝𝚞𝚛𝚊𝚝𝚎(𝚜𝚌𝚊𝚕𝚎⋅channels_temp1[2](I)⋅mat3(I))
		multiply(channels_temp2[1], mat3, channels_temp1[1]);  //此处scale为默认值1
		multiply(channels_temp2[0], mat3, channels_temp1[0]);
		merge(channels_temp1, mat1);  //与cv::split相反，合并多个数组到单个多通道数组
	}
	...
	void seamlessClone(InputArray _src, InputArray _dst, InputArray _mask, Point p, OutputArray _blend, int flags)
	{
		...
		Mat destinationROI = dst_mask(roi_d); // opencv中使用=复制Mat地址共享
